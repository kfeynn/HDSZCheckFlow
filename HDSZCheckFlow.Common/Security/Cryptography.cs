...
namespace HDSZCheckFlow.Common.Security
{
	/// <summary>
	/// Cryptography 的摘要说明。
	/// </summary>
	...
	{
		const string KEY_64 ="ChekFlow";
		const string IV_64 = "ChekFlow";
		...
		/// 加密
		/// </summary>
		/// <param name="inString">所要加密的数据</param>
		...
		public static string Encode(string inString)
		{
			...
			{
				Logger.Log("Cryptography->Encode",Ex.Message);  
			}

			outString = outString.Replace("=","%");          // = 转换为 % 
			outString = outString.Replace("+","_");          // + 转换为 - 解密时再转回来 ，作为url参数时不会出错
		...
		/// 解密
		/// </summary>
		/// <param name="inString">所要解密的数据</param>
		/// <returns>返回解密后的数据</returns>
		public static string Decrypt(string inString)
		{
			byte[] byKey = System.Text.ASCIIEncoding.ASCII.GetBytes(KEY_64);
			byte[] byIV = System.Text.ASCIIEncoding.ASCII.GetBytes(IV_64); 
			byte[] inputByteArray = new Byte[inString.Length];
			string outString = null;
			try
			{
				DESCryptoServiceProvider des = new DESCryptoServiceProvider();

				inString=inString.Replace("_","+");   //%号转换为 = ,_转换成 + 
			...
			{
				Logger.Log("Cryptography->Decrypt",Ex.Message); 
			}
