...
namespace HDSZCheckFlow.UI.Asset.PriceCheck
{
	...
	/// AddAssetApply 的摘要说明。
	/// </summary>
	public class AddPriceCheckApply : System.Web.UI.Page
	{
		#region
		...
		protected System.Web.UI.WebControls.Button btnMakeCheck;
		protected System.Web.UI.WebControls.Panel PShow;
		...
		private void bindGrid()
		{
			try
			{
				...
				oPageViewState.PSize=4;                   //返回参数,   页大小
		...
		{    
			...
			this.btnMakeCheck.Click += new System.EventHandler(this.btnMakeCheck_Click);
		...
		private void dgApply_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				//显示表体
				BindAssetBody(int.Parse(e.Item.Cells[0].Text));
		...
		private void bindSearchResult()
		{
			...
			AdvancedSearchGo("v_AssetApplyList_ForNonCheckPrice","applySheetHead_Pk",out oPageViewState.PCount,4,oPageViewState.PIndex,oPageViewState.SSort,oPageViewState.SortType,SQuery,sFields);
			//脚标
		...
		private string GetQuerySqlString()
		{
			...
			// 基础条件 ： 申请单已经审批完成 && 价格裁决未完成（已包装入视图功能） 。
		...
		private void btnMakeCheck_Click(object sender, System.EventArgs e)
		{
			//转到制作裁决表页面
			//传入参数 ：// 选定的表体ID 逗号分隔
			this.lbMsg.Text = "" ;
			...
			for(int i=0 ;i<this.dgApplyBody.Items.Count; i++)
			{
				CheckBox cbx =(CheckBox) this.dgApplyBody.Items[i].Cells[1].Controls[1] ; 
				...
				if(cbx.Checked )
				{
					...
					Ids += this.dgApplyBody.Items[i].Cells[0].Text ;
				}
			}
			...
			if(Ids.Length > 0 )
			{
				...
				Response.Redirect("AddPriceCheckApply_Setp2.aspx?Ids=" + Ids );
			}
			else
			{
				this.lbMsg.Text = "没有选择！";
			}
