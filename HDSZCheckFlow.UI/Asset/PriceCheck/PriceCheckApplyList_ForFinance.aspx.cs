//PriceCheckApplyList_ForFinance
using System;
...
{
	/// <summary>
	/// PriceCheckApplyList 的摘要说明。
	/// </summary>
	public class PriceCheckApplyList_ForFinance : System.Web.UI.Page
	{
		#region
		...
		protected System.Web.UI.WebControls.TextBox txtBargan;
		...
		private void bindSearchResult()
		{
			...
			AdvancedSearchGo("v_FinallyCheck_BodyInfo","Id",out oPageViewState.PCount,10,oPageViewState.PIndex,oPageViewState.SSort,oPageViewState.SortType,SQuery,sFields);
			//脚标
		...
		private string GetQuerySqlString()
		{
			...
			if(!"".Equals(this.txtDateFrom.Text))
			{
				...
				filter.Append("	makeDate >= " +" '" + this.txtDateFrom.Text+ "'" );
			}
			if(!"".Equals(this.txtDateTo.Text))
			{
				...
				filter.Append(" makeDate <= " +" '" +  this.txtDateTo.Text+ "'" );
			}
			...
			if(!"".Equals(this.txtBargan.Text))
			{
				if(filter.Length>0)
				{
					filter.Append(" and ");
				}
				filter.Append(" BargainNo like  " +" '%" + this.txtBargan.Text + "%'" );
			}
		...
		{
			//导出到Excel   v_FinallyCheck_BodyInfo

			string cmdStr = GetQuerySqlString();

			cmdStr = "select BargainNo 合同号,ApplySheetNo 单据号,MakeDate 申请日期,deptname 申请部门,currTypeCode 币种,ExchangeRate 汇率,Number 数量,FinallyPrice 单价,SubjectName 物品, ApplyProcessTypeName 单据状态,checkdate 最后审批日期,requestdate 到货日期,isArrivedC 标注到货 from v_FinallyCheck_BodyInfo where " + GetQuerySqlString();
