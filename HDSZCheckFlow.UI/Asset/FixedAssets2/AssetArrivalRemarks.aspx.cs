...
namespace HDSZCheckFlow.UI.Asset.FixedAssets2
{
	...
	public class AssetArrivalRemarks : System.Web.UI.Page
	{
			...
			if(!"".Equals(this.lblHidden.Text))
			{
				...
				if(AFBody != null )
				{
					if(isOrder == 0 && AFBody.IsInStore == 1 )
					{
						...
						this.lblMessage.Text = "已经办理入库，不能修改！";
					}
					else
					{
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			//导出到Excel   v_Asset_FinallyCheckBody

			string cmdStr = "" ;//GetQuerySqlString();

			cmdStr = " select applysheetno 订单号, BargainNo 合同号,deptname 申请部门,makedate 申请日期 ,InventoryName 项目名称,IsArrived 到货,ArriveSigner 标注人员,ArriveDatetime 到货时间 ,offer 供应商,number 数量,currtypecode 币种,Exchangerate 汇率, Finallyprice 价格,Exchangerate * number * Finallyprice as RMB金额 from v_Asset_FinallyCheckBody  where " + GetQuerySqlString();
