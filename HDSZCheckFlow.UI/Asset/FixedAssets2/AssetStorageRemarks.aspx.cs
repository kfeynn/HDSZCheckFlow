	...
	public class AssetStorageRemarks : System.Web.UI.Page
	{
			...
			try
			{
				if(e.CommandName.Equals("ReMark")) //点击审批按钮
				{
					...
					if(applyBody!=null)
					{
						...
						this.txtDate.Text = applyBody.InStoreDatetime ; 
		...
		{	
			if(!"".Equals(this.lblHidden.Text))
			{
				...
				if(AFBody != null )
				{
					if(isOrder == 0 && AFBody.IsPayFor == 1 )
					{
						this.lblMessage.ForeColor= Color.Red ;
						this.lblMessage.Text = "已经办理付款，不能修改！";
						
					}
					else
					{
					
						if(AFBody.IsArrived == 1   )
						{
							AFBody.IsInStore = isOrder ;
							AFBody.InStoreDatetime = this.txtDate.Text ;
							AFBody.InStoreSigner  = username ; 
							AFBody.Update();
