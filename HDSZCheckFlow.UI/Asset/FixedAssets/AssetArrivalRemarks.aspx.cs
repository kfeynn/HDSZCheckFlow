...
namespace HDSZCheckFlow.UI.FixedAssets
{
	...
	public class AssetArrivalRemarks : System.Web.UI.Page
	{
		...
		private void btnEnter_Click(object sender, System.EventArgs e)
		{
			...
			int  IsArrived = this.Checkbox2.Checked == true ? 1 : 0;
			//查询是否已经入库
			int IsInStore =  HDSZCheckFlow.Bussiness.AssetAdvanceRemarksBLL.SelectIsPayForByOS(this.Rownum);
			if(IsInStore < 1)
			{
				int UpAssetAdvance = HDSZCheckFlow.Bussiness.AssetAdvanceRemarksBLL.UadateAssetArrivalRemarks(IsArrived,EarnestSigner,txtDateTo,this.Rownum);
				if(UpAssetAdvance > 0)
				{
					this.lblMessage.Text = "标注验收成功！";				
					this.getPageViewLoad(this.Pages,getStrCondition(),0,0);
				}
				else
				{
					this.lblMessage.Text = "标注验收失败！";
					this.Label4.Text = "标注验收失败!";
				}
			}
			else
			{
				this.lblMessage.Text = "信息已经标注入库！";
				this.Label4.Text = "信息已经标注入库！";
			}
		...
		private void btnUpdateShuju_Click(object sender, System.EventArgs e)
		{
			foreach(DataGridItem itm in this.dgApply.Items )
			{
				...
				if( chkCode.Checked )
				{
					...
					if(IsInStore < 1)
					{
						int UpAssetAdvance =  HDSZCheckFlow.Bussiness.AssetAdvanceRemarksBLL.UadateAssetArrivalRemarks(Chi,Code,txtDateTo,ID);
						if(UpAssetAdvance < 1)
						{
					...
					{
						this.Label4.Text = "信息包含已经入库信息！请查看";
					}
