...
{
	/// <summary>
	/// PrintApply_Asset_FinallyPrice 的摘要说明。
	/// </summary>
	public class PrintApply_Asset_PriceCheckAll : System.Web.UI.Page
	{
		...
		private void dgApply_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				//RegisterStartupScript("onclick","<script>alert('别急！');</script>");
				//return;
				//给url参数加密，防止偷窥,Common.Security.Cryptography.Encode
				Response.Write("<script language='javascript'>window.open('PrintPage.aspx?applyPK=" + Common.Security.Cryptography.Encode(e.Item.Cells[1].Text) + " ');</script>");
