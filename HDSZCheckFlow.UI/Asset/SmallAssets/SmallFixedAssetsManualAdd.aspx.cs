...
namespace HDSZCheckFlow.UI.Asset.SmallAssets
{
	...
	/// </summary>
	public class SmallFixedAssetsManualAdd : System.Web.UI.Page
	{
		#region 
...
//			this.dgApply.AllowPaging=false;
			...
			string cmdStr = "select * from v_SmallFixedAssets_ForAll_OutPut";
...
//			this.dgApply.DataBind();
			...
			excelHelper.FileName="fileName.csv";
...
//			bindSearchResult();
		...
		{
			ddlInvName.SelectedIndex=0;//存货编码
			ddlDeptClassCode.SelectedIndex=0;//部门
			ddlDeptCode.SelectedIndex=0;//科组
			ddlCurrTypeCode.SelectedIndex=0;//币种
			txtINum.Text="";//起始数量
			txtInvManageCode.Text="";//管理编码
			txtPrice.Text="";//单价
			txtDbizdate.Text="";//购入日期
			txtStorage.Text="";//存放地点
			txtKeeperCode.Text="";//责任人工号
			txtReMark.Text="";//备注
		}
		...
		private void bindSearchResult2()
		{
			//查询的字段, * 为所有
			string sFields="*";
			//获取查询条件      
			SQuery=GetQuerySqlString2();
			//保存查询状态
			PageViewState oPageViewState=PageViewState.GetPageViewState(this.ViewState); 
			//参数意义:1.查询表或视图,必须有唯一列, 2.指明主键(唯一列) 3.输出参数 页大小 4.页尺寸(一页多少记录) 5.当前查询页码 6.排序字段 7.排序方式 8.查询条件 9.查询字段
			AdvancedSearchGo("v_SmallFixedAssets_ForAll","Id",out oPageViewState.PCount,10,oPageViewState.PIndex,oPageViewState.SSort,oPageViewState.SortType,SQuery,sFields);
		...
		private string GetQuerySqlString2()
		{
			#region  整理查询条件
			...
			filter.Append("  invmanagecode in (select invmanagecode from SmallFixedAssets group by invmanagecode having count(*) > 1)or invmanagecode is null  " );
