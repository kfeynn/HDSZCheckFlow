...
{
	/// <summary>
	/// SmallFixedAssetsForDept 的摘要说明。
	/// </summary>
	public class SmallFixedAssetsForDept : System.Web.UI.Page
	{
		...
		private void InitPageForAdd()
		{
			...
			{
				HDSZCheckFlow.Common.Log.Logger.Log("HDSZCheckFlow.UI.Asset.SmallAssets.SmallFixedAssetsForDept",ex.Message );
			}
		...
		private void bindGrid()
		{
			...
			{
				Common.Log.Logger.Log("SmallFixedAssetsForDept",ex.Message );
			}
		}
		...
		private string GetQuerySqlString()
		{
			...
			//string NCDeptCode = Bussiness.DeptBLL.GetMyNcDeptCode (classDept);

			if(filter.Length>0)
			{
				filter.Append(" and ");
			}
			filter.Append(" DeptClassCode in (" +" " + classDept + " ) " );
