...
namespace HDSZCheckFlow.UI.BaseData
{
	/// <summary>
	/// InputBaseDataFromExcel 的摘要说明。
	/// </summary>
	public class InputBaseDataFromExcel : System.Web.UI.Page
	{
			...
			try
			{
				...
				if(upload.IsAllowedExtension(this.File1))
				{
					...
					try
					{
						...
						string conStr="provider = Microsoft.Jet.OLEDB.4.0;Data Source=" + openFile + ";Extended Properties=\"Excel 8.0\";Persist Security Info=False";
		...
		private void btnAdd_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				switch(items)
				{
					...
					{
						this.lblMessage.Text="";
						DataSet ds=(DataSet)Cache["tempDS"];
						//预算导入
						Bussiness.BudgetAccountBLL.BudgetAccountHelper(ds,out lblMessage);
					...
					case "2":
					{
						...
						DataSet ds=(DataSet)Cache["tempDS"];
						//推算导入
						Bussiness.BudgetAccountBLL.BudgetTuisuanHelper(ds,out lblMessage);
					...
					case "3":           //新营预算 
					{
						...
						DataSet ds=(DataSet)Cache["tempDS"];
						//新营预算
						Bussiness.BudgetAccountBLL.AssetBudgetAccountHelper(ds,out lblMessage);
					...
					case "13":           //调整导入 , 暂时废弃 
					{
						...
						DataSet ds=(DataSet)Cache["tempDS"];
						//调整导入
						Bussiness.BudgetAccountBLL.BudgetChangeHelper(ds,out lblMessage);
