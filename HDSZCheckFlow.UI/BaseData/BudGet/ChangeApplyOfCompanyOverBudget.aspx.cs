...
namespace HDSZCheckFlow.UI.BaseData.BudGet
{
	...
	public class ChangeApplyOfCompanyOverBudget : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.TextBox txtApplyNo;//排序方式 1,升序.2降序
		...
		private void dgApply_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			try
			{
				...
				else if(e.CommandName.Equals("OverBudget"))  //设置可以预算外提交
				{
				...
				else if (e.CommandName.Equals("CanclOverBudget")) // 取消预算外提交 
				{
					...
					if(IsNewAddType(applyHeadPk))
					{
						Bussiness.ApplySheetHeadBLL.CancelSetIsOverBudget(applyHeadPk,0); // 参数, 单据Pk, 设置状态(1为可用)
			...
			{
				Common.Log.Logger.Log("设置可以预算外提交",ex.ToString());
			}
		}
		...
		private void btnEnter_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{				
					...
					if(ApplyBudget!=null)
					{
						...
						{
							this.lblMessage.Text = "预算外金额不能大于单据金额！";
						}
						else
						{
							...
							{
								Bussiness.ApplySheetHeadBLL.SetIsOverBudget(ApplyHead.ApplySheetHeadPk,1,overMoney); // 参数, 单据Pk, 设置状态(1为可用)
								this.lblMessage.Text = "设置成功！";
							}
						}
					}
				}
			...
			{
				this.lblMessage.Text = "出错！ "+ ex.Message ; 
