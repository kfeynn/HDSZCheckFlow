...
namespace HDSZCheckFlow.UI.BaseData.BudGet
{
	...
	/// </summary>
	public class DeptExpenseStates : System.Web.UI.Page
	{
	...
	//	public static int pagesIndex=1;		//列标头排序时,需要记住当前的页码
		...
		private string GetQuerySqlString()
		{
			...
			string myClassDept = Bussiness.DeptBLL.GetMyDeptClass(int.Parse(User.Identity.Name));

			if(filter.Length>0)
			{
				filter.Append(" and ");
			}
			filter.Append(" ApplyDeptClassCode = " +" '" + myClassDept + "'" );
