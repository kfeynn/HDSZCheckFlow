...
namespace HDSZCheckFlow.UI.BaseData.BudGet
{
	...
	/// </summary>
	public class ExpenseStates : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.DropDownList ddlExpense;
...
//				string cmdStr = "SELECT ApplyTypeCode, ApplyTypeName FROM ApplyType WHERE (ApplySheetBody_TableName = 'applysheetbody_pay')";
//				DataTable dtType = Bussiness.ComQuaryBLL.ExecutebyQuery(cmdStr).Tables[0];
//				if(dtType!=null && dtType.Rows.Count>0)
//				{
//					this.ddlType.DataSource =dtType;
//					ddlType.DataValueField=dtType.Columns[0].ToString();
...
//				}
		...
		private string GetQuerySqlString()
		{
			...
			if(!"".Equals(this.ddlExpense.SelectedValue))
			{
				if(filter.Length>0)
				{
					filter.Append(" and ");
				}
				if("1".Equals(this.ddlExpense.SelectedValue))
				{
					filter.Append (" isexpense = 1 ");
				}
				else
				{
					filter.Append (" ( isexpense != 1 or isexpense is null) ");
				}
			}
...
//			}
//
//			filter.Append (" ApplyTypeCode  in ( SELECT ApplyTypeCode FROM ApplyType WHERE (ApplySheetBody_TableName = 'applysheetbody_pay') ) ");
		...
		{	
			...
			if(ds!=null && ds.Tables.Count>0 && ds.Tables[0].Rows.Count>0)
			{
				...
				this.btnOutput.Visible= true;   //导出按钮可见
			}
			else
			{
				...
				this.btnOutput.Visible= false; //导出按钮不可见
			}
		}
		...
		{
			//导到Excel
			string cmdStr = "select *,(select top 1 item from dbo.ApplySheetBody_Pay a where a.applysheethead_pk = v_ApplySheetOfCompany.applysheethead_pk ) as item from v_ApplySheetOfCompany ";
