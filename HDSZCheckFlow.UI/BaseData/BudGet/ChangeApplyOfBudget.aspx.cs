...
namespace HDSZCheckFlow.UI.BaseData.BudGet
{
	...
	/// </summary>
	public class ChangeApplyOfBudget : System.Web.UI.Page
	{
		...
		private void dgApply_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			try
			{
				...
				else if(e.CommandName.Equals("btnChange"))  //调整单
				{
					int applyHeadPk = int.Parse(e.Item.Cells[0].Text); 

					Response.Redirect("ChangeOneApply.aspx?applyHeadPk= " + applyHeadPk + ""); // 转到调整单页面,

				}
				else if(e.CommandName.Equals("expense"))  //调整单
				{
					// 1. 设置此单为 已报销。 
					// 2. 设置报销日期为 当前时间 
					...
					if(applyHead!=null)
					{
						applyHead.IsExpense = 1 ;
						applyHead.ExpenseDate = DateTime.Now;
						applyHead.Update();
						bindGrid();
			...
			{
				Common.Log.Logger.Log("ChangeApplyOfBudget.dgApply_ItemCommand",ex.ToString());
			}
		}
		...
		{
			System.Web.UI.WebControls.Button    img=(System.Web.UI.WebControls.Button )e.Item.FindControl("btnExpense");   
			if   (!Object.Equals(img,null))   
			{   
				img.Attributes.Add("onclick","javascript:return   window.confirm('确认报销吗？')");   
			}   
