...
namespace HDSZCheckFlow.UI.OrderManage.BaseInfo
{
	...
	/// </summary>
	public class PickPurchase : System.Web.UI.Page
	{
		#region 
...
//				Cache.Remove("table");
		...
		private void bindGrid()
		{
			try
			{
				...
				oPageViewState.SSort="ApplySheetBody_Purchase_pk";
			...
			foreach(DataGridItem itm in this.dgApply.Items )
			{
				...
				if( chkCode.Checked )
				{
					...
					else
					{
						...
						dr["IsOrder"] = this.dgApply.Items[itm.ItemIndex].Cells[11].Text ; 
						dr["orderDate"] = this.dgApply.Items[itm.ItemIndex].Cells[12].Text ; 
						dr["OrderNo"] = this.dgApply.Items[itm.ItemIndex].Cells[13].Text ; 
						dr["IsGiveUp"] = this.dgApply.Items[itm.ItemIndex].Cells[14].Text ; 
		...
		private void imgBtnUp_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			foreach(DataGridItem itm in this.dgOrder.Items )
			{
				...
				if( chkCode.Checked )
				{
					...
					{
						if(dr["ApplySheetBody_Purchase_pk"].Equals(PurchasePk))
						{
							dr.Delete();
						}
					}
