...
namespace HDSZCheckFlow.UI.CheckFlow.CheckFlow
{
	...
	/// </summary>
	public class MyApplySheetInfo : System.Web.UI.Page
	{
		...
		protected System.Web.UI.HtmlControls.HtmlInputHidden HerdSort;//排序方式 1,升序.2降序
		...
		private void bindGrid()
		{
			...
			{
				Common.Log.Logger.Log("MyApplySheetInfo.bindGrid",ex.Message );
		...
		private string GetQuerySqlString()
		{
            ...
            if (!"".Equals(this.txtInvCode.Text))
            {
                if (filter.Length > 0)
                {
                    filter.Append(" and ");
                }
                filter.Append("  applysheethead_pk in (select applysheethead_pk from dbo.v_BaseApplyPurchase  where  inventorycode like    " + " '%" + this.txtInvCode.Text + "%') ");
            }

            if (!"".Equals(this.txtInvName.Text))
            {
                if (filter.Length > 0)
                {
                    filter.Append(" and ");
                }
                filter.Append("  applysheethead_pk in (select applysheethead_pk from dbo.v_BaseApplyPurchase  where  invName like    " + " '%" + this.txtInvName.Text + "%') ");
            }
			...
			// 控制只能查看本部门的单据 
			string classDept = Bussiness.DeptBLL.GetMyDeptClass(int.Parse(User.Identity.Name));
			if(filter.Length>0)
			{
				filter.Append(" and ");
			}
			filter.Append(" ApplyDeptClassCode = " +" '" + classDept + "'" );
		...
		private void linkToPage_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					Common.Log.Logger.Log("MyApplySheetInfo.linkToPage_Click11",ex.Message );
				}
				...
				pagesIndex.Value =oPageViewState.PIndex.ToString();
			...
			{
				Common.Log.Logger.Log("MyApplySheetInfo.linkToPage_Click",ex.Message );
			}
		}
		#endregion
