					...
					if("ApplySheetBody_Purchase".Equals(applyTypes.ApplySheetBodyTableName))
					{
						...
						{ 
							...
							{ 
								...
								if(number==0) //数量不能为0
								{
									XPGrid2.ShowMessage("数量不能为空或0！",System.Drawing.Color.Red);
									continueUpdate=false;
									return;
								}
						...
						//2014-03-21 更改。由存货编码 更新 币种、价格 信息。－－－－－－－－－－－－－－－－
						...
						if(dsPrice != null && dsPrice.Tables.Count > 0 && dsPrice.Tables[0].Rows.Count > 0 )
						{
							...
							{
								XPGrid2.ShowMessage("此物料没有维护单价！",System.Drawing.Color.Red);
								continueUpdate=false;
								return;
							}
							else
						...
						{
							XPGrid2.ShowMessage("对不起，此物料编码单价信息错误！",System.Drawing.Color.Red);
							continueUpdate=false;
						}		
						//－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
						...
						if(currType!=null)
						{
							...
							if (!MyChgSql.ChgResultSql(ref ResultSql,"ORIGINALCURRPRICE",RmbMoney.ToString())) //原币单价
							{
								XPGrid2.ShowMessage("对不起，系统错误！",System.Drawing.Color.Red); 
								continueUpdate=false;
							}
		...
		private void btnInConrt_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if(applyHead!=null)
				{
					...
					//************************做价格管理功能时可能出现的实际单价为空时也能提交的BUG，这里需要禁止这种情况提交（liyang 2014-08-13）**************************************
					//DataSet leftds = Bussiness.BudgetAccountBLL .getLeftMoneyForUnSubmitApply(int.Parse(keys[0]));		
