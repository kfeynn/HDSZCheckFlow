//	public class MyApplyFlowInfo : System.Web.UI.Page
using System;
...
namespace HDSZCheckFlow.UI.CheckFlow.CheckFlow
{
	...
	public class MyApplyFlowInfo : System.Web.UI.Page
	{
		...
		private void bindSearchResult()
		{
			...
			AdvancedSearchGo("v_AllApplyFlowInfo","ApplySheetHead_Pk",out oPageViewState.PCount,10,oPageViewState.PIndex,oPageViewState.SSort,oPageViewState.SortType,SQuery,sFields);
			//脚标
		...
		private string GetQuerySqlString()
		{
			...
			filter.Append("  CheckPersonCode  in (SELECT UserName FROM xpGrid_User  WHERE (displaysPerson = '" + userCode + "') AND (IsDisplays = 1) OR  (UserName = '" + userCode + "')) " );


//			(c.PersonCode IN (SELECT UserName FROM xpGrid_User  WHERE (displaysPerson = @UserCode) AND (IsDisplays = 1) OR  (UserName = @UserCode))) 
		...
		{	
			...
			{
				//////////////////////////////////////////////////
				// 处理DataSet 若 applySheetHead_pk相同,则只留下一条.
		...
		private void linkToPage_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					Common.Log.Logger.Log("MyApplyFlowInfo.linkToPage_Click1",pageIndex.ToString() + ex.Message );
				}
			...
			{
				Common.Log.Logger.Log("MyApplyFlowInfo.linkToPage_Click",ex.Message );
			}
		}
		#endregion
