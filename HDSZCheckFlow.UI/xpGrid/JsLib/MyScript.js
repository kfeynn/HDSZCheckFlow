...
/*--------------------------------以下脚本用于报表系统-----------------------------------------*/		
			...
			function Export2Excel()
				{
						...
						if (strCacheName != "")
							...
							parent.document.location.href=top.SystemRootPath+"AppSystem/ReptSystem/Export2Excel.aspx?CacheName=" +strCacheName;
							}
				}
					...
					if (jsNeedCleanCache==1)
						{
						 ...
						 wins=window.showModalDialog(MyUrl, window, "dialogWidth: 250px; dialogHeight: 100px;help: no;status:no");
						}
				}
	/*---------------------------------End-----------------------------------------*/				
	...
	/*以下代码用于将回车转为Tab键 及屏蔽退格键变成ctrl*/
			function setenter()
				...
				if ((event.keyCode==13) && ((event.srcElement.type == "text") || (event.srcElement.type == "password") || (event.srcElement.type == "checkbox") || (event.srcElement.type == "textarea") || (event.srcElement.type == "file") || (event.srcElement.type == "radio") ) ) {event.keyCode=9;};
				}
